...
class AutoRegisterConfig {
    ...
    void convertConfig() {
        registerInfo.each { map ->
            ...
            if (info.validate())
                ...
                project.logger.error('docking register config error: scanInterface, codeInsertToClassName and registerMethodName should not be null\n' + info.toString())
            }
    ...
    private void checkRegisterInfo() {
        ...
        if (!registerInfo.exists()) {
            ...
            if (!sameInfo) {
                project.logger.error("docking-register registerInfo has been changed since project(':$project.name') last build")
            }
        } else {
            project.logger.error('docking-register read registerInfo error--------')
        }
        if (!sameInfo) {
            deleteFile(AutoRegisterHelper.getRegisterCacheFile(project))
        }
        if (registerInfo.canWrite()) {
            ...
            project.logger.error('docking-register write registerInfo error--------')
        }
    }
